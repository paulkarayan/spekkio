...
Feature: Deposit Funds
  ...
  Background:
    Given an account exists with account_number "ACC001", holder_name "Alice", pin "1234", and balance 100.00

  @characterization @extracted @source:routes/transactions.py:26-38 @criterion:AC-005-01 @confidence:high
  Scenario: Successful deposit increases balance
    ...
    And the response body should contain "new_balance" with value 350.00
  ...
  Scenario: Deposit creates a transaction record
    ...
    And the transaction description should be "Deposit of $100.00"
